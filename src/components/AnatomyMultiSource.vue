...
var GTEX_API = 'https://test.gtexportal.org/rest/v1/'
var GTEX_VER = 'gtex_v8,kids_first_harmonization'
var GTEX_VER_DESCR = 'GTEx/Kids First Harmonization'
var AXIOS_CONF = { withCredentials: true }
...
export default {
  ...
  computed: {
    ...
    refTissue () {
      // return first non-Kids First tissue (currently no API support for topExpressedGenes on Kids First tissues)
      let rt = null
      if (this.gtexTissues != null) {
        this.gtexTissues.forEach(gt => {
          if ((rt == null) && (!gt.tissueSiteDetailId.match(/^KF.*/))) {
            rt = gt
          }
        })
      }
  ...
  watch: {
    ...
    tissue_info (ti) {
      ...
      let dt = {}
      ti.forEach(t => {
        t.source = t.tissueSiteDetailId.match(/^KF.*/) ? 'Kids First' : 'GTEx'

        // kids_first_harmonization uses different id format
        if (t.uberonId.toString().match(/^\d{4}$/)) {
          t.uberonId = '000' + t.uberonId
        }
      ...
      if (this.uberonIds) {
        ...
        tissues.forEach(gt => {
          this.gtexTissuesD[gt.tissueSiteDetailId] = gt
          gt.source = gt.tissueSiteDetailId.match(/^KF.*/) ? 'Kids First' : 'GTEx'
        })
    ...
    gtexTissues (ts) {
      ...
      let self = this
      axios.get(expnUrl, this.axiosConf).then(function (r) { self.setTopExpressedGenes(r.data.topExpressedGene) })
    },
    top_expressed_genes (teg) {
      ...
      axios.get(genesUrl, this.axiosConf).then(function (r) { self.setGenes(r.data.gene) })
    },
    tg2expression () {
  ...
  methods: {
    ...
    medianExpressionURL (genes, tissues) {
      ...
      // workaround for inability to filter by Kids First tissue - retrieve data for all tissues
      //          '&tissueSiteDetailId=' + encodeURIComponent(tissues.filter(t => !(t.tissueSiteDetailId.match(/^KF.*/))).map(x => x.tissueSiteDetailId).join(',')) +
    ...
    getTissueInfo (dataset) {
      ...
      let self = this
      axios.get(tissueUrl, this.axiosConf).then(function (r) { self.tissue_info = r.data.tissueInfo })
    },
